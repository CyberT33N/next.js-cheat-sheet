...
1. **`.env`** (all environments): Dies ist die Standarddatei für Umgebungsvariablen in Next.js. Die darin definierten Variablen gelten für alle Umgebungen, es sei denn, sie werden in spezifischeren Dateien überschrieben.
...
3. **`.env.development(next dev)`, `.env.production(next start)`, `.env.test`**: Diese Dateien enthalten Umgebungsvariablen, die spezifisch für die Entwicklungs-, Produktions- bzw. Testumgebungen sind. Die Variablen in diesen Dateien überschreiben die entsprechenden Variablen in der `.env`-Datei und in der `.env.local`-Datei, wenn vorhanden. Sie ermöglichen es Ihnen, Umgebungsvariablen je nach Umgebung anzupassen, ohne die Standarddatei `.env` zu ändern.
